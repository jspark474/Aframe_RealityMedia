(function () {
  ...
  */
  ...
  const DEFAULT_PROFILES_PATH$1 = 'https://cdn.jsdelivr.net/npm/@webxr-input-profiles/assets@1.0/dist/profiles';
  ...
  class XRControllerModelFactory {
  	...
  	constructor( gltfLoader ) {
  		...
  		this.path = DEFAULT_PROFILES_PATH$1;
  		this._assetCache = {};
  ...
  const LIB_URL = "https://cdn.jsdelivr.net/npm/handy-work" + ('@' + "1.5.0" );
    ...
    tick() {
      ...
      for (const inputSource of session.inputSources) {
        ...
        if (magnetTarget) {
          ...
          for (const bone of bones) {
            moveAroundAndTranslate(bone, magnetEl.object3D.position, tempQuaternion_A, tempVector3_A);
            bone.applyMatrix4(this.el.object3D.matrixWorld);
            bone.updateMatrixWorld();
          }
          ...
          for (const el of allEls.filter(el => el.dataset.noMagnet === undefined)) {
            moveAroundAndTranslate(el.object3D, magnetEl.object3D.position, tempQuaternion_A, tempVector3_A);
          }

          if (controllerModel) {
            moveAroundAndTranslate(controllerModel, magnetEl.object3D.position, tempQuaternion_A, tempVector3_A);
          }
        } else {
          for (const bone of bones) {
            bone.applyMatrix4(this.el.object3D.matrixWorld);
